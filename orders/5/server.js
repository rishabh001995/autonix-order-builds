...
const assetsBase = (process.env.CLIENT_ASSETS_BASE || '/uploads/website-client-assets/5').replace(/\/$/, '');
const clientLogoUrl =
  (process.env.CLIENT_LOGO_URL || '').trim() || `${assetsBase}/1776965099117-zd3al1hh-logo.png`;
const clientHeroUrl =
...
const defaultMeta =
  'Hippo is a food restaurant with an online menu and shop — order mains, sides, drinks, and treats for pickup or delivery where available.';
...
app.get('/sitemap.xml', (req, res) => {
  ...
  const urls = [
    ...
    { loc: `${base}/menu`, changefreq: 'weekly', priority: '0.95' },
...
app.get('/', (req, res) => {
  res.render('pages/home', {
    pageTitle: 'Hippo — Restaurant & online shop',
...
app.get('/menu', async (req, res, next) => {
  try {
    ...
    res.render('pages/menu', {
      pageTitle: 'Menu — Hippo',
      metaDescription:
        'Browse Hippo mains, sides, desserts, and drinks — same dishes you can add to your cart for easy ordering.',
      products,
    });
  } catch (e) {
    next(e);
...
app.get('/about', (req, res) => {
  res.render('pages/about', {
    pageTitle: 'About — Hippo',
    metaDescription:
      'Hippo is a food restaurant built around honest cooking, seasonal ingredients, and a calm forest-inspired dining room.',
  });
});
...
app.get('/shop', async (req, res, next) => {
  try {
    ...
    res.render('pages/shop-index', {
      pageTitle: 'Shop — Hippo',
      metaDescription: 'Order Hippo dishes online: bowls, burgers, flatbreads, salads, desserts, and drinks.',
      products,
    });
  } catch (e) {
    next(e);
...
app.get('/shop/:slug', async (req, res, next) => {
  try {
    ...
    if (!product) {
      return res.status(404).render('pages/404', { pageTitle: 'Not found — Hippo' });
    }
    res.render('pages/shop-product', {
      pageTitle: `${product.name} — Hippo`,
...
app.get('/cart', async (req, res, next) => {
  try {
    ...
    res.render('pages/cart', {
      pageTitle: 'Cart — Hippo',
      metaDescription: 'Review your Hippo order and head to checkout.',
      lines: summary.lines,
...
app.get('/checkout', async (req, res, next) => {
  try {
    ...
    res.render('pages/checkout', {
      pageTitle: 'Checkout — Hippo',
      metaDescription: 'Complete your Hippo order with delivery details and notes.',
      lines: summary.lines,
      subtotalCents: summary.subtotalCents,
  ...
  if (webhook) {
    try {
      await fetch(webhook, {
        ...
        body: JSON.stringify({
          source: 'autonix-order-5-checkout',
          email,
          fullName,
          totalCents: summary.subtotalCents,
...
app.get('/checkout/thanks', (req, res) => {
  res.render('pages/checkout-thanks', {
    pageTitle: 'Thank you — Hippo',
    metaDescription: 'Your Hippo order was received. We will confirm by email.',
  });
});

app.get('/contact', (req, res) => {
  res.render('pages/contact', {
    pageTitle: 'Contact — Hippo',
    metaDescription: 'Reach Hippo for reservations, catering questions, or help with your online order.',
  ...
  if (webhook) {
    try {
      await fetch(webhook, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          source: 'autonix-order-5-contact',
          name,
          email,
          company: company || null,
          message,
...
app.use((req, res) => {
  res.status(404).render('pages/404', { pageTitle: 'Not found — Hippo' });
});

app.use((err, req, res, next) => {
  console.error(err);
  res.status(500).render('pages/500', { pageTitle: 'Error — Hippo' });
});
...
async function start() {
  ...
  app.listen(PORT, () => {
    console.log(`autonix-order-5 listening on port ${PORT}`);
  });
}
